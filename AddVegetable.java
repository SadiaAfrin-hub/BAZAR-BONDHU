...
public class AddVegetable implements ActionListener {
    ...
    public AddVegetable() {
        ...
        frame = new JFrame("Add Vegetable");
        frame.setSize(600, 500);
        frame.setLayout(null);
        frame.getContentPane().setBackground(new Color(245, 245, 245));

        // Title Label
        JLabel titleLabel = new JLabel("Add Vegetable", JLabel.CENTER);
    ...
    private void addVegetable() {
        ...
        try {
            ...
            if (isIdExists(id)) {
                JOptionPane.showMessageDialog(frame, "Vegetable with the same ID already exists!", "ID Error", JOptionPane.ERROR_MESSAGE);
                clearFields();
                return;
            }

            // Save Vegetable data to file
            FileWriter writer = new FileWriter("vegetables.txt", true);
            writer.write(name + "," + id + "," + price + "," + instock + "," + photoPath + "\n");
            writer.close();

            JOptionPane.showMessageDialog(frame, "Vegetable added successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
            frame.dispose();
            ...
            JOptionPane.showMessageDialog(frame, "Error saving the Vegetable: " + ex.getMessage(), "File Error", JOptionPane.ERROR_MESSAGE);
        }
    }
