...
public class UpdateFruit {
    ...
    private String id;
    private ManageFruits manageFruits;

    public UpdateFruit(String id, ManageFruits manageFruits) {
        this.id = id;
        this.manageFruits = manageFruits;
        initialize();
    }

    private void initialize() {
        frame = new JFrame("Update Fruit");
        ...
        updateButton.addActionListener(e -> updateFruit());
        gbc.gridx = 0;
        gbc.gridy = 5;
        gbc.gridwidth = 2;
    ...
    private void loadFruitData() {
        try {
            ...
            if (!file.exists()) {
                JOptionPane.showMessageDialog(frame, "No fruits registered yet!");
                return;
            }
            ...
            JOptionPane.showMessageDialog(frame, "Error loading fruit data: " + ex.getMessage());
        }
    }
    ...
    private void updateFruit() {
        ...
        try {
            ...
            if (!updated) {
                JOptionPane.showMessageDialog(frame, "Fruit not found!");
                return;
            }
            ...
            JOptionPane.showMessageDialog(frame, "Fruit information updated successfully!");
            frame.dispose();
            manageFruits.displayFruits(); // Refresh the fruit data in ManageFruits
            new ManageFruits();
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(frame, "Invalid input for price or instock!");
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(frame, "Error updating fruit data: " + ex.getMessage());
        }
    }
}
